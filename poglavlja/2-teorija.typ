...
Побољшани глобални модел замењује локални модел на клијентским уређајима и постаје
почетна тачка за следећу рунду тренирања. Процес се изводи итеративно, чиме се
временом побољшавају перформансе дељеног модела, без употребе приватних података.

Један од кључних изазова и мера перформанси овог итеративног процеса јесте
директна веза између величине модела и системских трошкова. Већи, комплекснији
модели, попут модела _Gemma_ 3 @gemma3, нуде значајно бољу тачност предвиђања, али доносе
два практична оптерећења. Прво, захтевају веће рачунарске ресурсе на самом клијентском
уређају (меморију и процесорску снагу) за извођење локалног тренинга. Друго, генеришу
велика ажурирања тежина (која могу имати милијарде параметара), што ствара комуникационо
уско грло (_communication bottleneck_) приликом слања ажурирања са хиљада или милиона
уређаја на централни сервер.

Ефикасност федеративног учења не зависи само од алгоритма агрегације, већ и од техника
за компресију модела и смањење комуникационог оптерећења. Немогућност коришћења новијих,
боље оптимизованих модела, попут модела _Llama_ 3 @llama3, који су дизајнирани да понуде
бољи однос величине и перформанси, представља проблем који овај рад решава.
...
- Локална диференцијална приватност (_Local Differential Privacy_, LDP): Шум се додаје
    директно на сирове податке или ажурирања модела на самом уређају (клијенту) пре слања
    података на сервер, чиме се добија заштита и од злонамерног централног сервера.
...
- Централна диференцијална приватност (_Central Differential Privacy_, CDP): Шум се додаје
    ...
    Овај модел претпоставља да је сервер поуздан. Сервер агрегира све прикупљене доприносе
    и додаје шум на финални, агрегирани резултат пре него што га примени на глобални модел
    или га учини јавно доступним, чиме се штити приватност скупа података као целине, али се не
    штити приватност појединачних клијената од самог сервера.
...
гаранцију да чак ни ажурирања модела не могу открити податке појединаца.
У моделу где је централни сервер непоуздан, што је најчешћи сценарио када се штити приватност
корисника, примењује се локална диференцијална приватност (LDP).
У овом приступу, сваки клијент примењује ODP локално, на сопственом уређају, пре слања
података серверу. Клијент прво израчуна ажуриране тежине, затим клипује градијенте чиме ограничава
њихову осетљивост @dl_dp и на крају на њих додаје насумични шум. Тек тада се ове заштићене
тежине шаљу серверу.

Централни сервер, који врши агрегацију, никада не види осетљиве, оригиналне тежине
појединачних клијената, већ искључиво њихове заштићене верзије. На овај начин се
спречава да сервер може закључити било какве информације о појединачним тренинг подацима.
Ова комбинација је неопходна за изградњу модерних система машинског учења који су у
потпуности усклађени са регулативама за заштиту података.
...
попут платформе _Kaggle_-а @kaggle.

Док је верзија 2 библиотеке _Keras_ била чврсто везана за _TensorFlow_ као свој примарни _backend_, верзија 3
...
променљивим стањима оптимизатора (_optimizer slots_) приликом агрегације и дистрибуције.
