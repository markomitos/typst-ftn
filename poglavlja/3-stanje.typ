...
#figure([
    ...
    caption: [Исечак функције from\_keras\_model, који приказује имплементацију уније типова.]
)<lst:unija_tipova>

Поред примене уније типова примењена је и апстракција преко замене директних позива
са позивом помоћних функција из додате компоненте keras\_compat. На листингу @lst:unija_tipova
приказан је исечак функције functional_model_from_keras, који приказује додавање апстракције за
добављање промењљивих модела преко компоненте keras\_compat. Рефакторисан код је обележен бојама,
где је у црвеној боји је приказана код пре рефакторисања, док је зеленом бојом приказан код додат у рефакторисању.
