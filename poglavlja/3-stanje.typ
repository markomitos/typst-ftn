...
Библиотека _Keras_, разлог је проблема компатибилности библиотеке унутар радног окружења TFF.
Функција `tff.learning.from_keras_model`, била је дизајнирана да очекује
и користи унутрашње API позиве специфичне за _Keras_ 2 (нпр. за директно издвајање
_statefull_ променљивих оптимизатора). Пошто _Keras_ 3 не подржава ове позиве,
...
Рефакторисање се фокусирало на додавање логике компатибилности у TFF
...
(_model state variables_), методу `forward_pass` за прослеђивање података кроз модел
и методу `report_local_unfinalized_metrics `за прикупљање локалних статистика @tff.
...
(_server state_). Овај модул обезбеђује да TFF може исправно да серијализује и десеријализује
...
за имплементацију специфичних FL алгоритама, као што је федеративни просек (FedAvg) @tff. Због тога
...
својстава модела између верзија библиотеке, потребно је увести компоненту која
...
типа `tf_keras.Model` (имплементација _Keras_ 2) провера врши преко приватног атрибута `model._is_compiled`,
...
#figure(
  ...
  caption: [Функција `_is_compiled` из компоненте `keras_compat`]
)<lst:_is_compiled>
...
`tf.dtype` објекте. Пошто _Keras_ може да излаже типове података као једноставне `string`
...
def keras_dtype_to_tf(dtype_str):
  ...
  caption: [Функција `keras_dtype_to_tf` из компоненте `keras_compat`]
)<lst:keras_dtype_to_tf>

=== Функција `is_keras3`

Функција `is_keras3` утдрђује да ли прослеђени објекат (нпр. модел, променљива, метрика)
потиче из _Keras_ 3 библиотеке. Провера се врши коришћењем isinstance методе уграђене
...
#figure(
  ...
  caption: [Функција `is_keras3` из компоненте `keras_compat`]
)<lst:is_keras3>
...
#figure(
  ...
  caption: [Функција `get_optimizer_variables` из компоненте `keras_compat`]
...
које су апстрахованије, функција враћа _Python_ ID објекта користећи `id(variable)`.
Уколико прослеђена промењљива не припада ни једној од имплементација библиотеке _Keras_,
...
#figure(
  ...
  caption: [Функција `ref` из компоненте `keras_compat`]
)<lst:ref>
...
У библиотеци _Keras_ 3, променљиве су представљене класама које имају атрибут variable.value,
...
#figure(
  ...
  caption: [Функција `get_variable` из компоненте `keras_compat`]
)<lst:get_variable>
...
#figure(
  ...
  caption: [Функција `get_variables` из компоненте `keras_compat`]
)<lst:get_variables>
...
поуздано клонира модел за сваког клијента, одржавајући исту архитектуру уз нове тежине.
На листрингу @lst:clone_model приказан је код функције `clone_model`.
...
#figure(
  ...
  caption: [Функција `clone_model` из компоненте `keras_compat`]
)<lst:clone_model>

=== Функција `int_shape`

Функција `int_shape` враћа облик (`shape`) прослеђеног тензора или променљиве као торку целих
...
торке (`tuple`) у програмском језику _Python_, чиме се поједностављује логика за проверу типова
унутар радног окружења TFF. На листрингу @lst:int_shape приказан је код функције `int_shape`.
...
#figure(
  ...
  caption: [Функција `int_shape` из компоненте `keras_compat`]
)<lst:int_shape>

== Примена апстракције и уније типова
